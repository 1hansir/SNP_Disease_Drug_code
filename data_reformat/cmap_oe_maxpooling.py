...
# 此处不能使用gene_info 作为目录进行索引： gene_info中的基因均用于检测，并不是pert的基因
# 然而，pert的基因都是用hugo来储存，因此此处还需要将hugo symbol map到ensembl id上去
...
data_ge_maxpool.to_csv("../../CMap/Format_data/Maxpool/oe_all.csv",index_label=['gene'])
